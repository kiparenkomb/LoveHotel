(function () {
   ...
   * Build styles for application from SASS
   */
  gulp.task('buildJade', function () {// Название таска
  gulp.src('./*.jade')//берем файлы
    /*.pipe(plumber())*/
    .pipe(jade({pretty: true}))// компилим jade
    .pipe(gulp.dest(`./`));// куда положить html
  ...
  gulp.task('watch', function () {
    ...
    gulp.watch('./*.jade', ['buildJade']);
    watch('./*.jade', function () {
      gulp.run('buildJade');
    });
    gulp.watch([`./${Paths.build}/**/*`, './*.html', './*.jade']).on('change', function (file) {
  ...
  // Default Gulp Task
  gulp.task('default', ['buildCustomJS', 'buildSass', 'buildJsVendors', 'buildStylesVendors', 'buildJade', 'copyFonts', 'imageMin', 'startLocalhost', 'watch']);
  gulp.task('dev', ['buildCustomJS', 'buildSass', 'buildJsVendors', 'buildStylesVendors', 'buildJade', 'copyFonts', 'imageMin', 'watch']);
